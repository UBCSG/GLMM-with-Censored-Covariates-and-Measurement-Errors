...
# w_{ij} = w_{ij}^* + e_{ij}
# Random effects: alpha_{li} = alpha_l + a_{li}, l = 1,3,..,7
# alpha_{2i} = alpha_2 + gamma * age + a_{2i}
...
# Follow model: log(P(y_{ij}=1)/P(y_{ij}=0))=(beta0 + b_{0i}) + beta_w * w_{ij}^*
beta0 <- 8
beta1 <- -2.5
B <- 0.1

parnames <- c("beta0","beta1")
parnames_viral <- c("alpha1", "alpha2", "gamma", "alpha3", "alpha4","alpha5", "alpha6","alpha7")
...
bootstrap_par_new <- data.frame(matrix(nrow = num_boots, ncol = ncol))
colnames(bootstrap_par_new) <- parnames
bootstrap_par_new_viral <- data.frame(matrix(nrow = num_boots, ncol = ncol_viral))
colnames(bootstrap_par_new_viral) <- parnames_viral

bootstrap_par_SE_new <- data.frame(matrix(nrow = num_sim,ncol = ncol))
colnames(bootstrap_par_SE_new) <- parnames
...
true_value_viral <- c(alpha1, alpha2, gamma, alpha3, alpha4, alpha5, alpha6, alpha7)
...
for (i in 97:100){
  ...
  # simulate age, a_i, b_i, and e_ij
  ai_sim <- mvrnorm(num_patient, rep(0,dim(A)[1]), A)
  colnames(ai_sim) <- c("a1_sim","a2_sim","a3_sim","a4_sim","a5_sim","a6_sim","a7_sim")
  b0_sim <- rnorm(num_patient, 0, B)
  age_st <- rnorm(num_patient)
  ranef_sim <- cbind(PATIENT=c(1:num_patient), age_st, ai_sim, b0_sim)
  data <- merge(data0, ranef_sim, by="PATIENT", all = TRUE)
  data <- data %>% 
    mutate(e = rnorm(nrow(data), 0, sigma)) %>% 
    mutate(w_star = log10(exp(alpha1+a1_sim-(alpha2+gamma*age_st+a2_sim)*time)+exp(alpha3+a3_sim))*decay + ((alpha4+a4_sim) * (time-Ti) / ((time-Ti)  + exp(alpha5 + a5_sim - (alpha6+a6_sim) * (time-Ti))) + (alpha7 + a7_sim))*rebound) %>% 
  ...
  data <- data %>% 
    dplyr::select(c(PATIENT, time, age_st, decay, rebound, Ti, censor, limit, log10rna, CD4))
  data_new <- pivot_longer(data, cols = c(9:10), names_to = "name", values_to = "observation")
  data_new <- data_new %>% 
  ...
  write.table(data_new, "Chapter3/Data/Other/sim_data_new.txt", sep = ",", quote = FALSE, row.names = FALSE)
  write.table(data_naive, "Chapter3/Data/Other/sim_data_naive.txt", sep = ",", quote = FALSE, row.names = FALSE)
  write.table(data_empirical, "Chapter3/Data/Other/sim_data_empirical.txt", sep = ",", quote = FALSE, row.names = FALSE)
  
  # Our model 
  # Read in data for Monolix
  data_joint = list(dataFile = paste0('Chapter3/Data/Other/sim_data_new.txt'),
                    headerTypes =c("id","time", "contcov", "regressor", "regressor", "regressor","cens","limit","obsid","observation"),
  ...
  runScenario()
  # store the estimates
  estimates_new[i,] <- getEstimatedPopulationParameters()[9:10]
  SE_new[i,] <- as.numeric(getEstimatedStandardErrors()[["stochasticApproximation"]][["se"]][9:10])
  estimates_new_viral[i,] <- getEstimatedPopulationParameters()[1:8]
  SE_new_viral[i,] <- as.numeric(getEstimatedStandardErrors()[["stochasticApproximation"]][["se"]][1:8])
  
  
  # Naive model
  # Read in data for Monolix
  data_naive = list(dataFile = paste0('Chapter3/Data/Other/sim_data_naive.txt'),
                    headerTypes =c("id","ignore", "ignore", "ignore","ignore","ignore","ignore","ignore","contcov","observation"),
                    observationTypes =list(CD4 = "categorical"))
  modelFile = paste0('Chapter3/Model/model_naive.txt')
  # create a new project by setting a data set and a structural model
  newProject(data = data_naive, modelFile = modelFile)
  
  # covariates on beta50
  # getCovariateInformation()
  setCovariateModel(gamma0 = c(log10rna=TRUE))
  # set tasks in scenario
  scenario <- getScenario()
  scenario$tasks = c(populationParameterEstimation = T,
                     conditionalModeEstimation = T,
  ...
  # Empirical model
  # Read in data for Monolix
  data_empirical = list(dataFile = paste0('Chapter3/Data/Other/sim_data_empirical.txt'),
                        headerTypes =c("id","time", "contcov","ignore","ignore","ignore","ignore","ignore","obsid","observation"),
                        observationTypes =list(CD4 = "categorical", log10rna = "continuous"))
  modelFile = paste0('Chapter3/Model/model_empirical.txt')
  # create a new project by setting a data set and a structural model
  ...
  runScenario()
  # store the estimates
  estimates_empirical[i,] <- getEstimatedPopulationParameters()[5:6]
  SE_empirical[i,] <- as.numeric(getEstimatedStandardErrors()[["stochasticApproximation"]][["se"]][5:6])
  ...
  while(any(is.na(c(SE_empirical[i, ], SE_new[i, ], SE_naive[i, ], SE_new_viral[i, ])))){
    ...
    # Naive model 
    # data_naive <- read.table("Chapter3/Data/Other/sim_data_naive.txt", sep=",", header = TRUE)
  ...
  for (j in 1:num_boots){
    ...
    data <- data %>% 
      mutate(e = rnorm(nrow(data), 0, sigma)) %>% 
      mutate(w_star = log10(exp(estimates_new_viral[i, 1]+a1_sim-(estimates_new_viral[i, 2]+estimates_new_viral[i, 3] * age_st +a2_sim)*time)+exp(estimates_new_viral[i, 4]+a3_sim))*decay + ((estimates_new_viral[i, 5]+a4_sim) * (time-Ti) / ((time-Ti)  + exp(estimates_new_viral[i, 6] + a5_sim - (estimates_new_viral[i, 7]+a6_sim) * (time-Ti))) + estimates_new_viral[i, 8] + a7_sim)*rebound) %>% 
      mutate(log10rna = w_star + e) %>% 
      mutate(censor = ifelse(log10rna < censor_value, 1, 0)) %>% 
      mutate(log10rna = ifelse(log10rna < censor_value, censor_value, log10rna)) %>% 
      mutate(limit = 0) %>% 
    ...
    data <- data %>% 
      dplyr::select(c(PATIENT, time, age_st, decay, rebound, Ti, censor, limit, log10rna, CD4))
    data_new_boot <- pivot_longer(data, cols = c(9:10), names_to = "name", values_to = "observation")
    ...
    # Save the simulated data locally
    write.table(data_new_boot, "Chapter3/Data/Other/sim_data_new_boot.txt", sep = ",", quote = FALSE, row.names = FALSE)
    write.table(data_empirical_boot, "Chapter3/Data/Other/sim_data_empirical_boot.txt", sep = ",", quote = FALSE, row.names = FALSE)
    write.table(data_naive_boot, "Chapter3/Data/Other/sim_data_naive_boot.txt", sep = ",", quote = FALSE, row.names = FALSE)
    
    # Our model
    # Read in data for Monolix
    data_new_boot = list(dataFile = paste0('Chapter3/Data/Other/sim_data_new_boot.txt'),
                         headerTypes =c("id","time", "contcov","regressor", "regressor", "regressor","cens","limit","obsid","observation"),
                         observationTypes =list(CD4 = "categorical", log10rna = "continuous"))
    modelFile = paste0('Chapter3/Model/model_joint.txt')
    # create a new project by setting a data set and a structural model
    ...
    setCovariateModel(b1 = c(age_st = TRUE))
    setIndividualParameterDistribution(p1="normal",b1="normal",p2="normal",beta1="normal", beta2="normal",beta3="normal",beta4="normal", gamma0="normal", gamma1="normal")
    setPopulationParameterInformation(p1_pop = list(initialValue = estimates_new_viral[i, 1]),
                                      b1_pop = list(initialValue = estimates_new_viral[i, 2]),
                                      p2_pop = list(initialValue = estimates_new_viral[i, 4]),
                                      beta1_pop = list(initialValue = estimates_new_viral[i, 5]),
                                      beta2_pop = list(initialValue = estimates_new_viral[i, 6]),
                                      beta3_pop = list(initialValue = estimates_new_viral[i, 7]),
                                      beta4_pop = list(initialValue = estimates_new_viral[i, 8]),
                                      gamma0_pop = list(initialValue = estimates_new[i, 1]),
                                      gamma1_pop  = list(initialValue = estimates_new[i, 2]))
    # getVariabilityLevels()
    # setCorrelationBlocks(ID = list(c("b1", "p1", "b2", "p2","beta1","beta2","beta3")))
    # run the estimation
    setScenario(scenario)
    runScenario()
    # store the estimates
    bootstrap_par_new[j, ] <- getEstimatedPopulationParameters()[9:10]
    bootstrap_par_new_viral[j, ] <- getEstimatedPopulationParameters()[1:8]
  }
  bootstrap_par_SE_new[i,] <- sapply(bootstrap_par_new, sd)
  bootstrap_par_SE_new_viral[i,] <- sapply(bootstrap_par_new_viral, sd)
  
  saveRDS(estimates_new, "Chapter3/Data/Other/estimates_new_boot.RDS")
  saveRDS(estimates_new_viral, "Chapter3/Data/Other/estimates_new_viral_boot.RDS")
  saveRDS(estimates_naive, "Chapter3/Data/Other/estimates_naive_boot.RDS")
  saveRDS(estimates_empirical, "Chapter3/Data/Other/estimates_empirical_boot.RDS")
  saveRDS(SE_new, "Chapter3/Data/Other/SE_new_boot.RDS")
  saveRDS(SE_new_viral, "Chapter3/Data/Other/SE_new_viral_boot.RDS")
  saveRDS(SE_naive, "Chapter3/Data/Other/SE_naive_boot.RDS")
  saveRDS(SE_empirical, "Chapter3/Data/Other/SE_empirical_boot.RDS")
  saveRDS(bootstrap_par_SE_new,"Chapter3/Data/Other/bootstrap_par_SE_new_boot.RDS")
  saveRDS(bootstrap_par_SE_new_viral,"Chapter3/Data/Other/bootstrap_par_SE_new_viral_boot.RDS")
}  



estimates_new <- readRDS("Chapter3/Data/Other/estimates_new_boot.RDS")
SE_new <- readRDS("Chapter3/Data/Other/SE_new_boot.RDS")
bootstrap_par_SE_new  <- readRDS("Chapter3/Data/Other/bootstrap_par_SE_new_boot.RDS")
bootstrap_par_SE_naive  <- readRDS("Chapter3/Data/Other/bootstrap_par_SE_naive_boot.RDS")
bootstrap_par_SE_empirical  <- readRDS("Chapter3/Data/Other/bootstrap_par_SE_empirical_boot.RDS")
estimates_new_viral <- readRDS("Chapter3/Data/Other/estimates_new_viral_boot.RDS")
SE_new_viral <- readRDS("Chapter3/Data/Other/SE_new_viral_boot.RDS")
bootstrap_par_SE_new_viral  <- readRDS("Chapter3/Data/Other/bootstrap_par_SE_new_viral_boot.RDS")
estimates_naive <- readRDS("Chapter3/Data/Other/estimates_naive_boot.RDS")
SE_naive <- readRDS("Chapter3/Data/Other/SE_naive_boot.RDS")
estimates_empirical <- readRDS("Chapter3/Data/Other/estimates_empirical_boot.RDS")
SE_empirical <- readRDS("Chapter3/Data/Other/SE_empirical_boot.RDS")
...
saveRDS(result_new, "Chapter3/Results/SettingI/result_new_with_cov.RDS")
saveRDS(result_new_viral, "Chapter3/Results/SettingI/result_new_viral_with_cov.RDS")
saveRDS(result_naive, "Chapter3/Results/SettingI/result_naive_with_cov.RDS")
saveRDS(result_empirical, "Chapter3/Results/SettingI/result_empirical_with_cov.RDS")

result_new <- readRDS("Chapter3/Results/SettingI/result_new_with_cov.RDS")
result_new_viral <- readRDS("Chapter3/Results/SettingI/result_new_viral_with_cov.RDS")
result_naive <- readRDS("Chapter3/Results/SettingI/result_naive_with_cov.RDS")
result_empirical <- readRDS("Chapter3/Results/SettingI/result_empirical_with_cov.RDS")

table_names <- c("Method", "Parameter","True Value", "Estimates","SE$_{FIM}$", "SE$_{B}$","SE$_{MC}$",  "Bias (%)","rMSE (%)", "Coverage")
table <- data.frame(matrix(nrow=14, ncol=length(table_names)))
colnames(table) <- table_names
table$Parameter <- c("$\\alpha_1$","$\\alpha_2$", "$\\gamma$","$\\alpha_3$","$\\alpha_4$","$\\alpha_5$","$\\alpha_6$","$\\alpha_7$","$\\beta_0$",NA,NA,"$\\beta_w$",NA,NA)


table$`True Value` <- c(11, 0.3, -0.1, 2.8, 1.8, 8, 0.4, 1.3, 8,NA,NA,-2.5,NA,NA)
table$Method <- c("NLME",rep(NA,7), rep(c("NLME","Naive","LME"), 2))
...
print(latex_table, file = "Chapter3/Results/SettingI/latex_table_with_cov.tex",include.rownames=FALSE,sanitize.text.function = function(x){x},hline.after = c(-1,0,7, 14))
